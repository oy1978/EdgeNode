...
type SilValidator struct {
}

func NewSilValidator() *SilValidator {
	return &SilValidator{}
}

func (this *SilValidator) Run(req requests.Request, writer http.ResponseWriter) {
	...
	SilActionConfig, ok := actionConfig.(*SilAction)
	if !ok {
		req.ProcessResponseHeaders(writer.Header(), http.StatusTemporaryRedirect)
		http.Redirect(writer, req.WAFRaw(), originURL, http.StatusTemporaryRedirect)
		return
	...
	if req.WAFRaw().Method == http.MethodPost && len(req.WAFRaw().FormValue("GOEDGE_WAF_CAPTCHA_ID")) > 0 {
		this.validate(SilActionConfig, policyId, groupId, setId, originURL, req, writer)
	} else {
		// 增加计数
		SilIncreaseFails(req, SilActionConfig, policyId, groupId, setId, SilPageCodeShow)
		this.show(SilActionConfig, req, writer)
	}
}

func (this *SilValidator) show(actionConfig *SilAction, req requests.Request, writer http.ResponseWriter) {
	// show sil
	var countLetters = 6
	if actionConfig.CountLetters > 0 && actionConfig.CountLetters <= 10 {
		countLetters = int(actionConfig.CountLetters)
	}
	var silId = captcha.NewLen(countLetters)
	var buf = bytes.NewBuffer([]byte{})
	err := captcha.WriteImage(buf, silId, 200, 100)
	if err != nil {
		logs.Error(err)
		return
	}
	...
	<input type="hidden" name="GOEDGE_WAF_CAPTCHA_ID" value="` + silId + `"/>
	<div class="ui-image">
...
func (this *SilValidator) validate(actionConfig *SilAction, policyId int64, groupId int64, setId int64, originURL string, req requests.Request, writer http.ResponseWriter) (allow bool) {

	var silId = req.WAFRaw().FormValue("GOEDGE_WAF_CAPTCHA_ID")
	if len(silId) > 0 {
		var silCode = req.WAFRaw().FormValue("GOEDGE_WAF_CAPTCHA_CODE")
		if captcha.VerifyString(silId, silCode) {
			// 清除计数
			SilDeleteCacheKey(req)
			...
			// 增加计数
			if !SilIncreaseFails(req, actionConfig, policyId, groupId, setId, SilPageCodeSubmit) {
				return false
			}
